...
const fixUnknownCharacters = async () => {
    ...
    const posts = await moepics.search.posts({query: "solo +unknown-character +unknown-series", type: "image", rating: "all+h", style: "all+s", sort: "reverse date", limit: 99999})
...
const fixUnknownCharactersDanbooru = async () => {
    ...
    const posts = await moepics.search.posts({query: "+unknown-character +unknown-series", type: "image", rating: "all+h", style: "all+s", sort: "reverse date", limit: 99999})
    const tags = await moepics.tags.list([])
    let tagMap = {} as {[key: string]: Tag}
    for (const tag of tags) {
        tagMap[tag.tag] = tag
